...
vector<sf::Keyboard::Key>* InputCharacter::getListInputs()
{
...
void InputCharacter::setIndexJoystick(int indexJoystick)
{
    if (indexJoystick < -1)
        indexJoystick = -1;
    else if (indexJoystick > 7) //Limitation of SFML
        indexJoystick = 7;
    this->indexJoystick = indexJoystick;
}

int InputCharacter::getIndexJoystick()
{
    return this->indexJoystick;
}
...
{
    this->listInputs->at(indexAction) = key;
}

string InputCharacter::getKeyName(int indexAction)
{
    switch(this->listInputs->at(indexAction))
    {
    default:
